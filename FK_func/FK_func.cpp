...
void FK_function(const double P[3], double output1_data[], int output1_size[1],
                 double output2[3])
{
  ...
  cySq = P[0];
  Vn_idx_0 = P[1];
  Y_new_axis_idx_2 = P[2];
  for (int i{0}; i < 4; i++) {
    a[i] = (((b_a[i] * cySq + b_a[i + 4] * Vn_idx_0) +
             b_a[i + 8] * Y_new_axis_idx_2) +
            b_a[i + 12]) -
  ...
  Vn_idx_0 = a[1] * 0.0 - a[2] * -0.684;
  Vn_idx_1 = a[2] * -0.09 - a[0] * 0.0;
  Vn_idx_2 = a[0] * -0.684 - a[1] * -0.09;
  ...
  y = scale * std::sqrt(y);
  scale = 3.3121686421112381E-170;
  Y_new_axis_idx_2 = Vn_idx_0 / y;
  Vn_idx_0 = Y_new_axis_idx_2;
  absxk = std::abs(Y_new_axis_idx_2);
  if (absxk > 3.3121686421112381E-170) {
    cySq = 1.0;
    scale = absxk;
  } else {
  ...
  Y_new_axis_idx_2 = Vn_idx_1 / y;
  Vn_idx_1 = Y_new_axis_idx_2;
  absxk = std::abs(Y_new_axis_idx_2);
  if (absxk > scale) {
    t = scale / absxk;
    cySq = cySq * t * t + 1.0;
  ...
  cySq = scale * std::sqrt(cySq);
  Vn_idx_0 /= cySq;
  Vn_idx_1 /= cySq;
  Vn_idx_2 = Y_new_axis_idx_2 / cySq;
  ...
  Y_new_axis_idx_2 = B_Qbr[0] * Vn_idx_1 - Vn_idx_0 * B_Qbr[1];
  scale = 3.3121686421112381E-170;
  ...
  Vn_idx_0 = std::sqrt(cySq);
  output2[0] = rt_atan2d_snf(Y_new_axis_idx_2 / y, Vn_idx_2);
  output2[1] = rt_atan2d_snf(-B_Qbr[2], Vn_idx_0);
  output2[2] = rt_atan2d_snf(B_Qbr[1], B_Qbr[0]);
  if (cySq < 2.2204460492503131E-15) {
    cySq = rt_atan2d_snf(-B_Qbr[2], Vn_idx_0);
...
void main_FK_function()
{
    ...
    FK_function(dv, output1_data, *(int(*)[1]) & output1_size, output2);
    std::cout<<"Z Eulervinkel "<<output2[0]<<" Y Eulervinkel "<<output2[1]<<" X Eulervinkel "<<output2[2]<<std::endl;
    std::cout<<"X koordinat "<<output1_data[0]<<" Y koordinat "<<output1_data[1]<<" Z koordinat "<<output1_data[2]<<" Alle koordinater er i m"<<std::endl;
}
